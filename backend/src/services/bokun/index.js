...
const dialCodeByIso = new Map(
  allCountries.map((country) => [
    ...
    String(country.dialCode || country[2] || "")
  ])
);
...
const fetchCountries = async (requestId) => {
  const cacheKey = "countries";
  const cached = getCacheEntry(countryCache, cacheKey);
  if (cached) {
    return cached;
  }
  ...
  const countries = ensureArray(response)
    .map((country = {}) => {
      const code = String(country.isoCode || country.code || "").trim().toUpperCase();
      const title = String(country.title || country.name || code).trim();
      if (!code || !title) {
        return null;
      }
      ...
      return {
        code,
        dialCode: dialCodeByIso.get(code) ? `+${dialCodeByIso.get(code)}` : "",
        title,
        label: `${title} (${code})`
      };
    })
  ...
  return setCacheEntry(countryCache, cacheKey, countries, 24 * 60 * 60 * 1000);
};

const fetchProductPickupPlaces = async (productId, requestId) => {
  ...
  try {
    const response = await bokunClient.request({
      method: "get",
      path: `/activity.json/${encodeURIComponent(safeProductId)}/pickup-places?lang=EN&currency=${encodeURIComponent(env.DEFAULT_CURRENCY)}`,
      requestId
    });

    return mapper.mapActivityPickupPlaces(response);
  } catch (error) {
    logger.warn("Bokun pickup places unavailable", {
      requestId,
...
const fetchProductDetails = async (productId, requestId, options = {}) => {
  ...
  const [response, pickupPlaces] = await Promise.all([
    bokunClient.request({
      method: "get",
      path: `/activity.json/${safeProductId}?lang=EN&currency=${encodeURIComponent(env.DEFAULT_CURRENCY)}`,
      requestId
    }),
    fetchProductPickupPlaces(safeProductId, requestId)
  ]);

  const mapped = response ? mapper.mapProduct(response, { pickupPlaces }) : null;
...
const buildMainContactDetails = (customer = {}) =>
  [
    buildAnswer("firstName", customer.firstName),
...
const buildActivityPassengers = (payload = {}) => {
  ...
  const passengersFromCategories = categoryParticipants
    .map((participant) => {
      const pricingCategoryId = toNumberOrUndefined(participant.categoryId);
      const groupSize = Math.max(0, Number(participant.quantity || 0));

      if (!pricingCategoryId || groupSize < 1) {
        return null;
      }
  ...
  if (passengersFromCategories.length > 0) {
...
const buildActivityExtras = (extras = []) =>
  (Array.isArray(extras) ? extras : [])
    .map((extra) => {
      const extraId = toNumberOrUndefined(extra.code || extra.extraId || extra.id);
      const quantity = Math.max(0, Number(extra.quantity || 0));

      if (!extraId || quantity < 1) {
        return null;
      }
...
const buildCheckoutPayload = (payload = {}) => {
  ...
  const noteParts = [
    ...
    payload.startTime && !startTimeId ? `Requested start time: ${payload.startTime}` : ""
  ].filter(Boolean);
  ...
  if (startTimeId) {
    activityBooking.startTimeId = startTimeId;
  }
  ...
  const amount = Number(payload.amount || payload.totalPrice || 0);
  const currency = String(payload.currency || "USD").toUpperCase();
  const externalReference = String(payload.externalBookingReference || payload.bookingReference || "").trim();
  ...
  return {
    ...
    directBooking: {
      ...
      externalBookingReference: externalReference || undefined,
      externalBookingEntityName: "Riser Tours & Safaris",
      labels: ["website", "external-payment"],
    ...
    transactionDetails: {
      ...
      transactionId: String(payload.transactionId || externalReference || "")
    }
  };
};
...
const createBooking = async (payload, requestId) => {
  ...
  const checkoutResponse = await bokunClient.request({
    method: "post",
  ...
  const reservedBooking = checkoutResponse?.booking || checkoutResponse;
  const confirmationCode = reservedBooking?.confirmationCode || reservedBooking?.bookingReference || "";

  if (!confirmationCode || String(reservedBooking?.status || "").toUpperCase() === "CONFIRMED") {
    return mapper.mapBookingResponse(checkoutResponse);
  }

  const confirmedResponse = await bokunClient.request({
    method: "post",
    path: `/checkout.json/confirm-reserved/${encodeURIComponent(confirmationCode)}`,
    payload: {
      externalBookingReference:
        checkoutPayload.directBooking.externalBookingReference || reservedBooking.externalBookingReference || "",
      externalBookingEntityName: "Riser Tours & Safaris",
      showPricesInNotification: true,
      sendNotificationToMainContact: true,
      amount: checkoutPayload.amount,
      currency: checkoutPayload.currency,
      transactionDetails: checkoutPayload.transactionDetails
    },
    requestId
  });
