...
const buildBokunCreatePayloadVariants = ({ checkoutPayload = {}, liveQuote = {} }) => {
  ...
  const availabilitySlots = liveQuote.availability?.slots || [];
  const selectedSlot =
    availabilitySlots.find((slot) => slot.time === checkoutPayload.startTime) ||
    availabilitySlots.find((slot) => slot.status === "available" || slot.status === "limited") ||
    null;
  ...
  const basePayload = {
    ...
    startTimeId: checkoutPayload.startTimeId || selectedSlot?.startTimeId || "",
    bookingReference: checkoutPayload.bookingReference || "",
    externalBookingReference: checkoutPayload.externalBookingReference || checkoutPayload.bookingReference || "",
    transactionId: checkoutPayload.transactionId || "",
    amount: Number(checkoutPayload.amount || liveQuote.pricing?.finalPayable || liveQuote.pricing?.grossAmount || 0),
    currency: checkoutPayload.currency || liveQuote.pricing?.currency || "USD",
...
const resolvePickupPlaceId = ({ customer = {}, product = {} }) => {
  const directId = String(customer.pickupPlaceId || "").trim();
  if (directId) {
    return directId;
  }

  const selectedHotel = String(customer.hotelName || "").trim().toLowerCase();
  if (!selectedHotel) {
    return "";
  }

  const pickupPlace = (product.pickupPlaces || []).find((place) => {
    const placeId = String(place.id || "").trim();
    const label = formatPickupPlaceLabel(place).toLowerCase();
    const title = String(place.title || "").trim().toLowerCase();

    return (
      (placeId && selectedHotel === placeId.toLowerCase()) ||
      (label && selectedHotel === label) ||
      (title && selectedHotel === title) ||
      (label && label.includes(selectedHotel)) ||
      (title && selectedHotel.includes(title))
    );
  });

  return pickupPlace?.id ? String(pickupPlace.id) : "";
};
...
const upsertCustomer = async (customerPayload) => {
  ...
  if (existing) {
    ...
    existing.pickupPlaceId = customerPayload.pickupPlaceId || existing.pickupPlaceId;
  ...
  return Customer.create({
    ...
    pickupPlaceId: customerPayload.pickupPlaceId || "",
...
const persistBookingRecord = async ({
  ...
  if (paymentTransactionId !== undefined || dpoTransactionToken !== undefined) {
    bookingPatch.paymentTransactionId = resolvedTransactionId || undefined;
    // Backward compatibility for older records and callbacks.
    bookingPatch.dpoTransactionToken = resolvedTransactionId || undefined;
  }
...
const finalizePendingBookingAfterPayment = async ({
  ...
  if (!env.BOKUN_MOCK_MODE && isMockPaymentToken(resolvedTransactionToken)) {
    throw new AppError(
      "Mock payment cannot create or confirm a real Bokun booking",
      409,
      "MOCK_PAYMENT_NOT_ALLOWED_FOR_BOKUN",
      {
        bookingReference: initialBooking.bookingReference,
  ...
  try {
    ...
    const bokunCreateResult = await createBookingInBokunWithReliability({
      checkoutPayload: {
        ...
        amount: Number(liveQuote.pricing.finalPayable || liveQuote.pricing.grossAmount || booking.amount || 0),
        currency: liveQuote.pricing.currency || booking.currency || "USD"
      },
      liveQuote,
      requestId,
      bookingReference: booking.bookingReference
...
const cancelBooking = async ({ id, reason, auth, requestId }) => {
  ...
  if (auth?.role === "agent") {
    if (String(booking.agentId || "") !== String(auth.id || "")) {
      throw new AppError("Booking not found", 404, "BOOKING_NOT_FOUND");
    }
    ...
    booking.editRequests.push(cancelRequest);
    ...
    await AuditLog.create({
      ...
      metadata: {
        bookingReference: booking.bookingReference,
        sourceChannel: booking.sourceChannel || "agent_portal"
      }
    });
...
const requestBookingEdit = async ({ id, payload, reason, auth, requestId }) => {
  ...
  if (auth?.role === "agent" && String(booking.agentId || "") !== String(auth.id || "")) {
    throw new AppError("Booking not found", 404, "BOOKING_NOT_FOUND");
  }
