...
const mapPickupPlaces = (pickupPlaceGroups = []) => {
  const places = [];
  const seen = new Set();

  ensureArray(pickupPlaceGroups).forEach((group) => {
    const groupTitle = stripHtml(group?.title || group?.name || "");

    ensureArray(group?.pickupPlaces).forEach((place) => {
      const title = stripHtml(place?.title || place?.name || "");
      if (!title) {
        return;
      }

      const id = String(place?.id || place?.pickupPlaceId || title).trim();
      ...
      places.push({
        ...
        latitude: place?.latitude || place?.geoPoint?.latitude || null,
        longitude: place?.longitude || place?.geoPoint?.longitude || null
      });
    });
  });
...
const mapActivityPickupPlaces = (activityPlaces = {}) => {
  const places = [];
  const seen = new Set();

  ensureArray(activityPlaces?.pickupPlaces).forEach((place) => {
    const title = stripHtml(place?.title || place?.name || "");
    if (!title) {
      return;
    }
    ...
    const wholeAddress = String(location.wholeAddress || "").trim();
    const countryCode = String(location.countryCode || "").trim();
    const wholeAddressHasCountry =
      countryCode && wholeAddress.toLowerCase().split(/\s|,/).includes(countryCode.toLowerCase());
    const address = wholeAddress
      ? compactJoin([wholeAddress, wholeAddressHasCountry ? "" : countryCode])
      : compactJoin([location.address, location.city, countryCode]);
    const id = String(place?.id || place?.pickupPlaceId || title).trim();
    const key = `${id}:${title}`.toLowerCase();
    if (seen.has(key)) {
      return;
    ...
    places.push({
      id,
      title,
      groupTitle: place?.type || "",
      address,
      latitude: location.latitude || null,
      longitude: location.longitude || null,
      externalId: place?.externalId || "",
      type: place?.type || "",
      askForRoomNumber: Boolean(place?.askForRoomNumber)
    });
  });
...
const mapProduct = (rawProduct = {}, options = {}) => {
...
const mapActivityAvailability = ({ payload = {}, rawAvailabilities = [], priceList = null, defaultCurrency = "USD" }) => {
  ...
  internalSlots.forEach((slot) => {
    ...
    if (!existing.startTimeId && slot.startTimeId) {
      existing.startTimeId = slot.startTimeId;
    }
...
const mapBookingResponse = (raw = {}) => {
  ...
  return {
    bokunBookingId: booking.id || booking.bookingId,
    bookingReference:
      booking.externalBookingReference ||
      booking.bookingReference ||
      booking.reference,
    confirmationCode: booking.confirmationCode || booking.bookingReference || "",
