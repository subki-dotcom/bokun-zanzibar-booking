...
const env = cleanEnv(process.env, {
  ...
  ACCOUNTING_BASE_CURRENCY: str({ default: "USD" }),
  EXPENSE_POSTING_MODE: str({ default: "PREVIEW_ONLY" }),
  EXPENSE_AUTOMATIC_POSTING_ENABLED: bool({ default: false }),
  SUPPLIER_PAYMENT_AUTOMATIC_POSTING_ENABLED: bool({ default: false }),
  AR_AUTOMATIC_RECOGNITION_ENABLED: bool({ default: false }),
  REVENUE_RECOGNITION_AUTOMATIC_ENABLED: bool({ default: false }),
  REVENUE_RECOGNITION_ACTIVATED_AT: str({ default: "" }),
  REVENUE_RECOGNITION_ACTIVATION_SCOPE: str({ default: "" }),
  CUSTOMER_PAYMENT_AUTOMATIC_POSTING_ENABLED: bool({ default: false }),
  EXPENSE_POSTING_BATCH_SIZE: num({ default: 25 }),
  ...
  BOKUN_WEBHOOK_IMPORT_MISSING: bool({ default: false }),
  ...
  BOKUN_CONFIRMED_BOOKING_IMPORT_ENABLED: bool({ default: true }),
  BOKUN_CONFIRMED_BOOKING_IMPORT_INTERVAL_SECONDS: num({ default: 900 }),
  BOKUN_CONFIRMED_BOOKING_IMPORT_BATCH_SIZE: num({ default: 50 }),
  BOKUN_CONFIRMED_BOOKING_IMPORT_MAX_PAGES: num({ default: 5 }),
  BOKUN_CONFIRMED_BOOKING_IMPORT_LOOKBACK_DAYS: num({ default: 30 }),
  BOKUN_CONFIRMED_BOOKING_IMPORT_STATUSES: str({ default: "CONFIRMED,CANCELLED" }),
  BOOKING_FINALIZATION_RETRY_ENABLED: bool({ default: true }),
  BOOKING_FINALIZATION_RETRY_INTERVAL_SECONDS: num({ default: 30 }),
  BOOKING_FINALIZATION_RETRY_BATCH_SIZE: num({ default: 20 }),
  BOOKING_FINALIZATION_MAX_RETRIES: num({ default: 8 }),
  INVOICE_SYNC_ENABLED: bool({ default: true }),
  INVOICE_SYNC_INTERVAL_SECONDS: num({ default: 300 }),
  INVOICE_SYNC_BATCH_SIZE: num({ default: 100 }),
  REFUND_RECONCILIATION_ENABLED: bool({ default: true }),
  REFUND_RECONCILIATION_INTERVAL_MINUTES: num({ default: 10 }),
  REFUND_RECONCILIATION_INTERVAL_SECONDS: num({ default: 0 }),
  REFUND_RECONCILIATION_BATCH_SIZE: num({ default: 20 }),
  REFUND_RECONCILIATION_MIN_AGE_SECONDS: num({ default: 300 }),
  REFUND_RECONCILIATION_MAX_RETRIES: num({ default: 8 }),
  ...
  PAYMENT_SAME_CURRENCY_TOLERANCE: str({ default: "0.005" }),
  ...
  DPO_APPROVED_CURRENCIES: str({ default: "USD" }),
