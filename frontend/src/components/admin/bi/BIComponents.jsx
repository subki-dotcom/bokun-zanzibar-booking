...
export function BIProductProfitability({ rows, currency }) {
  ...
  return <div className="bi-table-scroll" role="region" aria-label="Product profitability; scroll horizontally for all columns" tabIndex={0}><table><thead><tr><th>Product</th><th>Bookings</th><th>Revenue</th><th>Posted direct cost</th><th>Actual direct cost</th><th>Gross profit</th><th>Margin</th></tr></thead><tbody>{rows.map((row, index) => { const templateCoverageComplete = Number(row.templateBookings || 0) >= Number(row.bookings || 0); return <tr key={row.productId || index}><td><strong>{row.productTitle || row.productId || 'Unmapped product'}</strong>{!hasValue(row.grossProfit) && !templateCoverageComplete && <small className="bi-warning-text">Cost coverage incomplete</small>}</td><td>{formatNumber(row.bookings)}</td><td>{formatMoney(row.revenue, currency)}</td><td>{formatMoney(row.directCost, currency)}</td><td>{formatMoney(row.actualDirectCost, currency)}</td><td>{formatMoney(row.grossProfit, currency)}</td><td>{formatPercentage(row.margin)}</td></tr>; })}</tbody></table></div>;
}
