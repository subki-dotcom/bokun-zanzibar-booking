...
const AdminCancellationPolicyCard = ({ policy = null, currency = "USD" }) => {
  ...
  const fee = hasKnownRefundAmount(policy.estimatedCancellationFee)
    ? formatCurrency(policy.estimatedCancellationFee, currency)
    : "Manual review";
  const refund = hasKnownRefundAmount(policy.estimatedRefundAmount)
    ? formatCurrency(policy.estimatedRefundAmount, currency)
    : "Manual review";
  return (
    <Card className="surface-card mb-3">
      <Card.Body>
        ...
        <div className="booking-request-detail-grid">
          ...
          <DetailRow label="Submitted in free window" value={policy.isFreeCancellationAvailable ? "Yes" : "No"} />
          <DetailRow label="Manual review" value={policy.requiresManualReview ? "Required" : "Not required"} />
          <DetailRow label="Cancellation fee" value={fee} />
          <DetailRow label="Estimated refund" value={refund} />
        </div>
    ...
    <div className="booking-request-details-page">
      ...
      <Row className="g-3 align-items-start">
        <Col xl={8}>
          ...
          {request.type === "cancel_booking" ? <AdminCancellationPolicyCard policy={request.cancellationPolicySnapshot} currency={currency} /> : null}
