...
const statusVariant = (value = "") => (["completed", "approved", "synced", "refunded", "partially_refunded", "paid", "eligible"].includes(value) ? "success" : ["rejected", "failed", "cancelled", "unavailable"].includes(value) ? "danger" : "warning");
...
const CancellationRefundCard = ({ request, booking, invoice, payments, refundContext, refundSummary, currency }) => {
  ...
  const amountPaid = refundSummary?.amountPaid ?? refundContext?.amountPaid ?? invoice?.amountPaid ?? request.originalSnapshot?.amountPaid ?? 0;
...
const AdminBookingRequestDetailsPage = () => {
  ...
  const { request, payments = [], invoice, audit = [], refundContext, refundSummary } = data;
  ...
  const refundIsComplete = ["refunded", "partially_refunded"].includes(String(refundSummary?.status || refund?.status || ""));
  const refundCanVerify = Boolean(refund?._id) && !refundIsComplete && (refundProvider === "pesapal" || Boolean(refund?.providerRefundReference));
      ...
      : request.bokunSync?.status !== "synced" || booking.bookingStatus !== "cancelled" ? "Waiting for Bokun cancellation confirmation."
        : refundIsComplete ? "Refund already confirmed."
          : String(refund.status || "") === "processing" ? "Refund is already processing."
            : String(refund.status || "") === "awaiting_merchant_approval" ? "Pesapal accepted the request; merchant confirmation is pending."
          ...
          {refund ? (
            <Card className="surface-card mb-3">
              <Card.Body>
                ...
                <DetailRow label="Reference" value={refund.refundReference} />
                <DetailRow label="Approved amount" value={formatCurrency(refund.amount, refund.currency || currency)} />
                <DetailRow label="Provider" value={resolvedRefundProviderLabel} />
                <DetailRow label="Status" value={label(refundSummary?.status || refund.status)} />
                <DetailRow label="Confirmed refunded" value={formatCurrency(refundSummary?.amountRefunded ?? refund.confirmedRefundedAmount ?? 0, refundSummary?.currency || refund.currency || currency)} />
                ...
                <Button
                  ...
                  variant="success"
                  disabled={Boolean(busy) || !refundCanProcess}
                  onClick={() => {
                    if (!window.confirm(`Process refund of ${formatCurrency(refund.amount, refund.currency || currency)} via ${resolvedRefundProviderLabel}?`)) return;
                    run("process-refund", () => processBookingRequestRefund(refund._id, { notes: refundNotes }), "Refund processing updated.");
                  }}
                >
                  Process Refund - {formatCurrency(refund.amount, refund.currency || currency)} via {resolvedRefundProviderLabel}
                </Button>
                {refundDisabledReason ? <small className="text-muted d-block mt-2">{refundDisabledReason}</small> : null}
                <Button
                  className="w-100 mt-2"
                  variant="outline-primary"
                  disabled={Boolean(busy) || !refundCanVerify}
                  onClick={() => run("verify-refund", () => verifyBookingRequestRefund(refund._id), "Refund status verified.")}
                >
                  <BsArrowRepeat /> Verify Refund Status
                </Button>
                {!refundCanVerify && !refundIsComplete ? <small className="text-muted d-block mt-2">Provider refund reference is required before automatic verification for this provider.</small> : null}
                <Form.Control className="mt-3" value={refundNotes} onChange={(event) => setRefundNotes(event.target.value)} placeholder="Optional refund processing note" />
                <hr />
                <Form.Select className="mt-3" value={refundStatus} onChange={(event) => setRefundStatus(event.target.value)}>
                  <option value="processing">Processing</option>
                  <option value="awaiting_merchant_approval">Awaiting merchant approval</option>
                ...
                <Form.Control className="mt-2" value={refundReference} onChange={(event) => setRefundReference(event.target.value)} placeholder="Provider refund reference" />
                <Form.Control className="mt-2" type="number" min="0" step="0.01" value={confirmedAmount} onChange={(event) => setConfirmedAmount(event.target.value)} placeholder="Confirmed amount, only for completed refund" />
                <Form.Control className="mt-2" as="textarea" rows={2} value={refundEvidenceNote} onChange={(event) => setRefundEvidenceNote(event.target.value)} placeholder="Evidence note for manual completion" />
                <Button
                  className="mt-2"
                  variant="outline-success"
                  disabled={Boolean(busy)}
                  onClick={() => run("refund", () => updateBookingRequestRefund(refund._id, { status: refundStatus, providerRefundReference: refundReference, evidenceNote: refundEvidenceNote, ...(confirmedAmount !== "" ? { confirmedAmount: Number(confirmedAmount) } : {}) }), "Refund status updated.")}
                >
                  Record Manual Refund Update
                </Button>
                <small className="d-block text-muted mt-2">Use manual update only after provider or merchant portal confirmation.</small>
              </Card.Body>
            </Card>
          ) : null}
